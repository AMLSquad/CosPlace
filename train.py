...
if __name__ == '__main__':
	...
	# Attention Module
	attention_module = 'something'

	# Model
	model = network.GeoLocalizationNet(args.backbone, args.fc_output_dim, 'Some')
	...
	classifiers = [cosface_loss.MarginCosineProduct(
		args.fc_output_dim, len(group)) for group in groups]
	classifiers_optimizers = [torch.optim.Adam(
		classifier.parameters(), lr=args.classifiers_lr) for classifier in classifiers]

	logging.info(f"Using {len(groups)} groups")
	logging.info(
		f"The {len(groups)} groups have respectively the following number of classes {[len(g) for g in groups]}")
	logging.info(
		f"The {len(groups)} groups have respectively the following number of images {[g.get_images_num() for g in groups]}")

	val_ds = TestDataset(args.val_set_folder,
	...
	if args.resume_train:
		model, model_optimizer, classifiers, classifiers_optimizers, best_val_recall1, start_epoch_num = \
			util.resume_train(args, output_folder, model,
							model_optimizer, classifiers, classifiers_optimizers)
	...
	for epoch_num in range(start_epoch_num, args.epochs_num):
		...
		classifiers[current_group_num] = classifiers[current_group_num].to(
			args.device)
