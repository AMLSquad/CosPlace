...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, class_num):
        ...
        image_path = random.choice(self.images_per_class[class_id])#random.choice(self.images_per_class[class_id])
        ...
        tensor_image = torch.load(image_path, map_location= torch.device('cpu')).squeeze(0)
        
        return tensor_image, class_num, filename, da_label
    ...
    def initialize(dataset_folder, M, N, alpha, L, min_images_per_class, filename, pseudo_target_folder):
        ...
        images_paths = sorted(glob(f"compact/**/*.jpg", recursive=True))
